...
| **Assistant chat** | Assistant-guided creation. Pick a goal (**Newsletter, Product update, Promotion or Win-back**) and the assistant drafts content, subject lines and preview text. See [Building with the assistant](/en/guide/building-with-ai). Consumes [AI credits](/en/guide/ai-credits) and needs a connection (offline switches to the Editor). |
...
To stop a **Scheduled** campaign, edit or delete it before its send time. Once sending has started, it cannot be stopped from the app. See [Troubleshooting](/en/guide/troubleshooting) for the full checklist.
